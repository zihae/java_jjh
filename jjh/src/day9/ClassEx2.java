 ...
 * 멤버 메소드: 
 * -현재 좌표 정보를 출력하는 메소드
 * -지정된(->누군가 알려준) 좌표로 이동하는 메소드
 * 생성자
 * -기본 생성자: x, y 좌표를 0으로 초기화
 ...
 */
	...
	class Point2D{
		...
		Point2D(){
			x=0; y=0; //기본 생성자. 이 줄을 지워도 됨=> 멤버변수(파랑색) x,y는 기본 초기값이 0
		}
		Point2D(int x1, int y1){ //매개변수 (갈색)가 있는 생성자
			x=x1; y=y1;	
		}
		 ...
		 * 매개변수: 지정된 좌표(x1, y1) -> int x1, int y1
		 * 리턴타입: 없음 -> void
		 * 메소드명: move
		 */
