...
public class ClassBoardEx1 {
	...
	public static void main(String[] args) {
		/* 게시글을 관리하기 위한 Board 클래스를 생성하고, 객체를 생성하여 테스트 해보세요. */
		  
		Board [] boardList = new Board[10];
		//게시글 관리 프로그램
		int menu = 0;
		int count = 0; //현재까지 저장된 게시글 개수
		...
		do {
			...
			case 1 : 
				...
				date = scan.next();
				//(아래)게시글을 생성한 후, 입력받은 정보들로 초기화를 하고, 초기화된 게시글을 내가 원하는 배열에 번지에 저장
				boardList[count] = new Board(count+1, count, title, contents, date, writer);
				//저장된 게시글 하나 증가
				count++;
				break;
			case 2 :
				...
				num = scan.nextInt();
				if(num <= count) {
					//게시글 삭제여부를 확인해서 삭제된 게시글이면 삭제되었습니다 라고 출력하고, 아니면 게시글 조회
					if(boardList[num-1] == null) {
						System.out.println("삭제된 게시글입니다.");
					}
					else {
						//조회수 증가 (조회수 확인하기 전에 증가시킴)
