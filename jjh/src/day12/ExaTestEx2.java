...
public class ExaTestEx2 {
	...
	public static void main(String[] args) {
		 ...
		 * 사각형과 타원을 합쳐서 최대 10개까지 저장하는 배열( 타입(->도형 클래스) 배열명 [] = new 타입 [10]; )
		 * 다형성 예제
		 * */
		//ExaRect rlist [] = new ExaRect[10];
		//ExaEllipse elist [] = new ExaEllipse[10];	
		ExaShape list [] = new ExaShape [10];   //사각형과 타원을 *합쳐서* 최대 10개까지 저장하는 배열
		//타원 1개와 사각형 2개를 생성하여 배열에 저장
		//클래스 형변환, 자식 => 부모 클래스로 형변환, 자동 클래스 형변환
		...
		/* NullPointerException : 객체가 없는데 객체의 기능을 사용하려고 할 때 발생하는 에러
		 * 객체가 없다 ==> null */
		for(int i =0; i<list.length; i++) {
		/*	
		 * 도형이 있으면 print() 기능을 이용하여 출력하고, 도형이 없으면 아무것도 안함
		 * 도형이 없으면 아무것도 안함
		 * 도형이 없다가 null인 이유
		...
		for(ExaShape tmp : list) {
			if(tmp != null) {
				//도형이 타원이면 testE()를 호출하고, 사각형이면 testR()을 호출하세요. 
		...
		/* 기능 :매개변수가 사각형이면 사각형이라고 출력하고, 매개변수가 원이면 원이라고 출력하고, 매개변수가 도형이면 도형이라고 
		 * 		도형이라고 출력하는 메소드
		 * 매개변수: 사각형 또는 원 또는 도형 => 도형 =>ExaShape shape
		 * 리턴타입: 없음
		 * 메소드명: print 
		 */
	
		public static void print(ExaShape shape) {
			if(shape instanceof ExaRect) { //조건식이 다 달라서 switch문으로 사용할 수 없음
