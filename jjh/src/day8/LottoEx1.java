...
public class LottoEx1 {
	...
	public static void main(String[] args) {
		...
		//입력 확인 코드 System.out.println(입력 번호 : " + Arrays.toString(user));"
		scan.close();
		//등수를 출력
		/* 1등: 당첨번호 6개 전부 일치
	...
	/* 방법 2.
	 * 기능: 배열의 크기가 주어지면 주어진 배열의 크기만큼 min에서 max 사이의 랜덤한 수를 생성하여 저장한 후 만들어진 배열을 알려주는 메소드
	 * 매개변수: 정수 범위(min~max), 배열의 크기 => int min, int max, int size
	 * 리턴타입: 배열 => int []		  
	 * 메소드명: randomLotto
	 */
	...
	public static int createBonus(int min, int max, int [] arr) {
		...
		while(true) {
			...
			//당첨 번호에 보너스 번호가 있으면 조건식으로 점프(이동)하여 다시 번호 생성
	...
	/* 기능: 두 배열이 주어지면, 두 배열에서 일치하는 정수의 개수를 알려주는 메소드
	 * 매개변수: 두 배열 => int [] arr1, int [] arr2
	 * 리턴타입: 일치하는 정수의 개수 => 정수 => int
	 * 메소드명: check
	 */
	...
	public static void rank(int [] lotto, int bonus, int [] user) {
		...
		case 6: result = "1등 당첨!!";	
			break;
		case 5: result = containsArray(user, bonus, user.length) ? "2등 당첨!!" : "3등 당첨!!";	
			break;
		case 4: result = "4등 당첨!!";	
			break;
		case 3: result = "5등 당첨!!"	;
