...
<body>
	<div class="body container">
		<c:if test="${board != null}">
			...
			<c:if test="${board.bd_type != 'notice' && board.bd_num == board.bd_ori_num }">
	...
	<script>
			$(function(){
				...
				$('.btn-comment').click(function(){
					...
					var user = '${user.me_id}';
					//아이디가 없으면 로그인 안 한 상태이기 때문에 로그인하라고 알려줌
					if(user == ''){
					...
					//ajax로 댓글 정보를 보내기 위해 객체를 만듦
					//이 때 만들어지는 객체의 속성명을 CommentVO의 멤버 변수 이름과 일치시킴
					var comment = {
					...
					$.ajax({
						...
						success : function(res){
							...
							if(res == true){
								...
								//새로고침(전체가 아닌 댓글부분만)을 하여 1페이지에 맞는 댓글을 가져옴
			...
			$(document).on('click','.comment-list .btn-del-comment', function(){
				//삭제할 댓글 번호는 삭제 버튼에 data-num 속성값으로 입력되어 있음
			...
			$(document).on('click','.comment-list .btn-mod-comment2', function(){
				...
				//수정된 댓글 내용과 번호를 이용하여 객체를 만듦.서버로 전송하기 위해
				var comment = {
						co_num : co_num, 
						co_contents : co_contents
						}
			...
			$(document).on('click','.btn-reply-comment',function(){
				...
				'</div>';
			//등록 버튼을 위한 html(답글 등록을를 위한 버튼)
			var btnStr = '<button class="btn-reply-comment2 btn btn-outline-warning ml-2">'+'답글 등록'+'</button>'
			//textarea태그를 아이디 밑에 배치
			...
			function readComment(co_bd_num, page){
				...
				if(co_bd_num != ''){
					$.ajax({
						...
						//서버에서 보낸 데이터의 타입
						dataType:"json",
						//ajax가 성공적으로 완료되면 서버에서 보낸 댓글 리스트와 페이지 메이커를 res에 담음
						success : function(res){
							//res.list : 댓글 리스트(페이지 번호에 맞는)
							//res.pm : 댓글의 페이지 메이커
							var str = '';
					    ...
					    //서버에서 보낸 페이지메이커를 이용하여 html로 이루어진 페이지네이션을 만듦
